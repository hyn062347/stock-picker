...
export async function POST(req) {
  try {
    ...
    // 즐겨찾기 추가
    await pool.query(
      "INSERT INTO favorite (user_id, symbol) VALUES ($1, $2)",
      [userId, symbol]
    );
...
export async function GET(req) {
  try {
    ...
    const result = await pool.query(
      "SELECT symbol FROM favorite WHERE user_id = $1",
      [userId]
    );
...
export async function DELETE(req) {
  try {
    ...
    await pool.query(
      "DELETE FROM favorite WHERE user_id = $1 AND symbol = $2",
      [userId, symbol]
    );
