...
import { fetchForeignHoldingsTable, type ForeignHoldingRow } from "../n_finance/service";
import { fetchNaverNewsArticles, type NaverArticle } from "../n_news/service";
...
async function buildFinancialContext(symbol: string): Promise<FinancialContext> {
  ...
  const insiderTransactions =
    (summary as Record<string, unknown> | undefined)?.insiderTransactions &&
    typeof (summary as any).insiderTransactions === "object"
      ? ((summary as any).insiderTransactions.transactions ?? [])
      : [];
...
async function main() {
  ...
  if (!symbol) {
    console.error("No symbol provided. Usage: pnpm ts-node src/app/api/rec_generate/service.ts <SYMBOL>");
    process.exit(1);
  }
